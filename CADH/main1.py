...
#迭代数据
    ...
    for epoch in range(epochs): # 50
        ...
        #掩码损失#设置掩码块，并初始化一个mask的实例
            ...
            mask_generator = FeatureMasker(mask_ratio=args.mask_ratio, block_size=args.block_size)#
        ...
        #分类损失
            ...
            #编码器损失
         #mmd损失
            #mmd_loss = MMDLoss(kernel_type='rbf', kernel_mul=args.kernel_mul, kernel_num=args.kernel_num)#
            ...
            #量化损失可以多加入一个目标与的损失
            loss_hash = hash_pairwise_loss(hash_source_data, hash_target_data, sim_matrix, alpha=args.alpha)#  0.0008               
    ...
    save_path = '/media/abc/ware/uad/program5_tiv/file/mnist/ptor2.pth'
...
if __name__ == "__main__":
    ...
    # base_path  = '/media/abc/ware/uad/data/use_data_tiv/mnist/'#'/media/abc/ware/uad/data/use_data_tiv/office_home/''/media/abc/ware/uad/data/use_data/'
    ...
    if args.dataset == 'Office-Home':  # (1) Art (2) Clipart (3) Product (4) Real_World
        base_path = '/media/abc/ware/uad/data/use_data_tiv/office_home/'#'/media/abc/ware/uad/data/use_data_tiv/office_home/'
        ...
        base_path = '/media/abc/ware/uad/data/use_data_tiv/office31/'
        source_loader, n_class, dim1 = get_source_data(base_path, source_domain,args.batchsize)
        target_loader = get_target_data(base_path, target_domain,args.batchsize)
        target_train_loader = target_loader['train']
        target_test_loader = target_loader['query']
        ...
        base_path = '/media/abc/ware/uad/data/use_data_tiv/mnist/'
        source_loader, n_class, dim1 = get_source_data(base_path, source_domain,args.batchsize)
        target_loader = get_target_data(base_path, target_domain,args.batchsize)
        target_train_loader = target_loader['train']
        target_test_loader = target_loader['query']
